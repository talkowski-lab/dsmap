...
mu.hist <- function(mu, cnv=NULL, x.axis.title=NULL, y.axis.title="Loci",
  ...
  sapply(x.ax.at, function(x){
    axis(1, at=x, tick=F, line=-0.65, labels=bquote(10 ^ .(x)),
         cex.axis=0.85)
  })
...
option_list <- list(
  ...
  make_option(c("--x-title"), help="Custom X-axis title",
              type="character", default=NULL),
  make_option(c("--y-title"), help="Custom Y-axis title",
              type="character", default=NULL)
)

# Get command-line arguments & options
args <- parse_args(OptionParser(usage="%prog mu.tsv out_prefix",
